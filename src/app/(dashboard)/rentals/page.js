          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                {session?.user?.role === "TRADESMAN" && <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Actions</th>}
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200">
              {filtered.map((rental) => (
                <tr key={rental.id}>
                  ...
                  {session?.user?.role === "TRADESMAN" && (
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      {rental.status === "PENDING" && (
                        <div className="flex gap-2">
                          <button onClick={() => updateStatus(rental.id, "APPROVED")} className="text-green-600 font-medium hover:text-green-700">Approve</button>
                          <button onClick={() => updateStatus(rental.id, "REJECTED")} className="text-red-600 font-medium hover:text-red-700">Reject</button>
                        </div>
                      )}
                      {rental.status === "APPROVED" && (
                        <button onClick={() => updateStatus(rental.id, "COMPLETED")} className="text-blue-600 font-medium hover:text-blue-700">Complete</button>
                      )}
                      <button onClick={() => deleteRental(rental.id)} className="text-gray-500 ml-2 hover:text-gray-700">Delete</button>
                    </td>
                  )}
                </tr>
              ))}
